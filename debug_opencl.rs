...
fn test_pbkdf2_kernel(context: &Context, queue: &Queue) -> Result<(), Box<dyn std::error::Error>> {
    ...
    kernel.set_arg(0, &password_buffer)?;
    kernel.set_arg(1, mnemonic.len() as i32)?;
    kernel.set_arg(2, &salt_buffer)?;
    kernel.set_arg(3, salt.len() as i32)?;
    kernel.set_arg(4, &seed_buffer)?;
    kernel.set_arg(5, 1i32)?;
...
fn test_bip44_kernel(context: &Context, queue: &Queue) -> Result<(), Box<dyn std::error::Error>> {
    let source = fs::read_to_string("src/kernels/bip44.cl")?;
    let program = Program::builder()
        .devices(queue.device())
        .src(source)
        .build(context)?;
    ...
    kernel.set_arg(0, &seed_buffer)?;
    kernel.set_arg(1, &private_key_buffer)?;
    kernel.set_arg(2, &public_key_buffer)?;
    kernel.set_arg(3, &success_buffer)?;
    kernel.set_arg(4, 1i32)?;
...
fn test_secp256k1_kernel(context: &Context, queue: &Queue) -> Result<(), Box<dyn std::error::Error>> {
    ...
    kernel.set_arg(0, &private_key_buffer)?;
    kernel.set_arg(1, &public_key_buffer)?;
    kernel.set_arg(2, &success_buffer)?;
    kernel.set_arg(3, 1i32)?;
...
fn test_keccak_kernel(context: &Context, queue: &Queue) -> Result<(), Box<dyn std::error::Error>> {
    ...
    kernel.set_arg(0, &public_key_buffer)?;
    kernel.set_arg(1, &address_buffer)?;
    kernel.set_arg(2, 1i32)?;
